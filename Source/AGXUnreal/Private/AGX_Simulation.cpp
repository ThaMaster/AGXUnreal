...
TArray<FSensorContactData> UAGX_Simulation::GetSensorContactData(const FShapeBarrier& Shape) const
{
	if (HasNative())
	{
		return NativeBarrier.GetSensorContactData(Shape);
	}

	return TArray<FSensorContactData>();
}
