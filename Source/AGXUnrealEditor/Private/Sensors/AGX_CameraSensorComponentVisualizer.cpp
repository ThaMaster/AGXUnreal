...
{
	const UAGX_CameraSensorComponentBase* Cam = Cast<const UAGX_CameraSensorComponentBase>(Component);
	if (Cam == nullptr || !Cam->ShouldRender())
		return;

	if (!UAGX_CameraSensorComponentBase::IsFovValid(Cam->FOV))
		return;

	if (!UAGX_CameraSensorComponentBase::IsResolutionValid(Cam->Resolution))
		return;
