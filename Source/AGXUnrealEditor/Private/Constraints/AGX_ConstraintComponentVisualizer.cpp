...
namespace
{
		...
		float Scale, uint8 DepthPriority, float Thickness, float DepthBias, bool bScreenSpace)
	{
		...
		PDI->DrawLine(
			AxisLoc, AxisLoc + X * Scale, FLinearColor::Red, DepthPriority, Thickness, DepthBias, bScreenSpace);
		PDI->DrawLine(
			AxisLoc, AxisLoc + Y * Scale, FLinearColor::Green, DepthPriority, Thickness, DepthBias, bScreenSpace);
		PDI->DrawLine(
			AxisLoc, AxisLoc + Z * Scale, FLinearColor::Blue, DepthPriority, Thickness, DepthBias, bScreenSpace);
	}
	...
	const UActorComponent* Component, const FSceneView* View, FPrimitiveDrawInterface* PDI)
{
	...
	{
		/// Hack to force update of render transform, if for example the constraint uses a constraint transform actor
		/// and it was moved without UAGX_ConstraintDofGraphicsComponent knowing about it...
	...
	const AAGX_Constraint* Constraint, const FSceneView* View, FPrimitiveDrawInterface* PDI)
{
	...
	for (int RigidBodyIndex = 0; RigidBodyIndex < 2; ++RigidBodyIndex)
	{
		...
		if (AActor* RigidBodyActor = BodyAttachment.RigidBodyActor)
		{
			...
			{
				FVector Direction = (RigidBodyActor->GetActorLocation() - View->ViewLocation).GetSafeNormal();
	...
	if (bDrawLineBetweenActors)
	{
		...
		if (RigidBodyActor1 && RigidBodyActor2)
		{
			...
			FVector Direction1 = (RigidBodyActor1->GetActorLocation() - View->ViewLocation).GetSafeNormal();
			FVector Location1 = View->ViewLocation + Direction1 * Distance;

			FVector Direction2 = (RigidBodyActor2->GetActorLocation() - View->ViewLocation).GetSafeNormal();
			FVector Location2 = View->ViewLocation + Direction2 * Distance;

			DrawDashedLine(
				PDI, Location1, Location2, HighlightColor, HighlightThickness, SDPG_Foreground, /*DepthBias*/ 0.0f);
		}
	}
}

void FAGX_ConstraintComponentVisualizer::DrawConstraintHUD(
	const AAGX_Constraint* Constraint, const FViewport* Viewport, const FSceneView* View, FCanvas* Canvas)
{
